...
# PHASE2_ONLY=test_name[,test_name] narrows the run when diagnosing one
# failure; PHASE2_TRACEBACK=1 prints whole tracebacks instead of one line.
ONLY = [t for t in os.environ.get("PHASE2_ONLY", "").split(",") if t]
FULL_TB = os.environ.get("PHASE2_TRACEBACK") == "1"

def _select(suite):
    """Flatten a suite, keeping only tests named in ONLY."""
...
def run(orm_shim, kernel, mode):
    ...
    Running both in one interpreter and diffing them looked obvious and is
    wrong: these suites leak state across a second load of the same modules,
    so whichever ran second picked up four failures that neither mode has on
    its own. The delta then measured the contamination, not the routing. The
    parent process runs this twice and diffs the two JSON payloads.
    """
...
fn compare() -> Result<()> {
    ...
    for (name, b) in baseline.as_object().into_iter().flatten() {
        ...
        if !bp.is_empty() {
            println!("    pre-existing in both modes: {bp:?}");
        }
    }
