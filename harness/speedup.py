...
def main() -> None:
    ...
    print(
        f"median per-case speedup: {statistics.median(values):.2f}x   (one vote per case)"
    )
    print(
        f"time-weighted speedup:   {weighted:.2f}x   "
        "(total python / total rust over the same cases)"
    )
