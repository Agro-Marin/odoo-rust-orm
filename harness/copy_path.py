...
COLUMNS = ["name", "ref", "comment", "is_company", "color", "birthdate", "partner_latitude"]
...
with Registry(dbname).cursor() as cr:
    ...
    if type(cr._cnx).__name__ != "FakeConnection":
        failures.append("the db shim did not take: cursor is %s" % type(cr._cnx).__name__)
...
JSON_CODES = ["X0", "X1", "X2", "X3", "X4", "X5", "X6", "X7",
              "X8", "X9", "Y0", "Y1", "Y2", "Y3", "Y4"]
JSON_CASES = [
...
try:
    ...
    if json_rows != len(JSON_CASES):
        failures.append("jsonb: wrote %d countries, read back %d"
                        % (len(JSON_CASES), json_rows))
...
from odoo.orm.runtime.backend import COPY_THRESHOLD  # noqa: E402

copies = db_shim.pool_stats().get("copies", 0)
print("COPY streams opened through the rust cursor: %d (threshold %d, %d rows)"
...
print("COPY %s (%d rows, %d columns each)"
