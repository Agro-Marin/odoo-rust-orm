...
tokio-postgres, `NUMERIC` decoding and all. The db shim now has its own
`ACTIVE` flag, set through `rust_db_shim.set_active` from the same mode by
`rust_engine._set_mode` at start and on every kill-switch tick: while it is
off the pool factory builds psycopg pools, while it is on it builds rust ones
(`on` and `shadow` both need them — the kernel runs inside the caller's
