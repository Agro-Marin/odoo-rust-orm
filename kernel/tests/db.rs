//! Tests that need a PostgreSQL to talk to.
//!
//! Ignored by default. Run them with the database they may use:
//!
//! ```text
//! RUSTORM_TEST_DSN='host=/var/run/postgresql user=me dbname=scratch' \
//!     cargo test -p odoo-kernel --test db -- --ignored
//! ```
//!
...
//! and leave nothing behind, and they never need an Odoo database.
...
async fn a_restricting_rule_is_read_from_the_forks_column() {
    ...
    let by_id: Vec<(Vec<i32>, bool, bool)> = rules
        .iter()
        .map(|r| (r.groups.clone(), r.restrict, r.is_granting_group()))
        .collect();
    assert_eq!(
        by_id,
        vec![
